       ...
       <div className="rounded-xl border border-border bg-card p-6 shadow-sm mb-8">
        <div className="mb-6 flex items-center justify-between border-b border-border pb-4">
          <div className="flex items-center gap-3">
             <div className={`rounded-xl p-2 ${editingId ? 'bg-amber-500/10' : 'bg-primary/10'}`}>
              {editingId ? <Pencil className={`h-6 w-6 ${editingId ? 'text-amber-500' : 'text-primary'}`} /> : <PlusCircle className="h-6 w-6 text-primary" />}
            </div>
        ...
        <form onSubmit={handleCriteriaSubmit} className="space-y-4">
          ...
          {/* ... ALL FORM FIELDS AND LOGIC HAVE BEEN PORTED DOWN ... */}
           <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="mb-1 block text-sm font-medium text-foreground">ปีการศึกษา</label>
               <input type="number" required value={criteriaForm.academic_year} onChange={e => setCriteriaForm({...criteriaForm, academic_year: e.target.value})} className="h-10 w-full rounded-md border border-input bg-background text-foreground px-3 outline-none focus:border-primary focus:ring-1 focus:ring-primary" />
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium text-foreground">รอบ TCAS (1-4)</label>
               <select required value={criteriaForm.tcas_round} onChange={e => setCriteriaForm({...criteriaForm, tcas_round: e.target.value})} className="h-10 w-full rounded-md border border-input bg-background text-foreground px-3 outline-none focus:border-primary focus:ring-1 focus:ring-primary">
          ...
          <div className="grid grid-cols-2 gap-4">
             <div>
              <label className="mb-1 block text-sm font-medium text-foreground">จำนวนรับ (ที่นั่ง)</label>
              <input type="number" required min="1" value={criteriaForm.max_seats} onChange={e => setCriteriaForm({...criteriaForm, max_seats: e.target.value})} className="h-10 w-full rounded-md border border-input bg-background text-foreground px-3 outline-none focus:border-primary focus:ring-1 focus:ring-primary" />
             </div>
            <div>
               <label className="mb-1 block text-sm font-medium text-foreground">GPAX ขั้นต่ำ</label>
              <input type="number" step="0.01" min="0" max="4" required value={criteriaForm.min_gpax} onChange={e => setCriteriaForm({...criteriaForm, min_gpax: e.target.value})} className="h-10 w-full rounded-md border border-input bg-background text-foreground px-3 outline-none focus:border-primary focus:ring-1 focus:ring-primary" />
             </div>
          </div>
          
           <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="mb-1 block text-sm font-medium text-foreground">วันที่เริ่มต้นรับสมัคร <span className="text-destructive">*</span></label>
               <input type="date" required value={criteriaForm.start_date} onChange={e => setCriteriaForm({...criteriaForm, start_date: e.target.value})} className="h-10 w-full rounded-md border border-input bg-background text-foreground px-3 outline-none focus:border-primary focus:ring-1 focus:ring-primary" />
            </div>
             <div>
              <label className="mb-1 block text-sm font-medium text-foreground">วันที่สิ้นสุดรับสมัคร <span className="text-destructive">*</span></label>
               <input type="date" required value={criteriaForm.end_date} onChange={e => setCriteriaForm({...criteriaForm, end_date: e.target.value})} className="h-10 w-full rounded-md border border-input bg-background text-foreground px-3 outline-none focus:border-primary focus:ring-1 focus:ring-primary" />
            </div>
          </div>

           <div className="border-t border-border pt-4">
            <label className="mb-2 block text-sm font-medium text-foreground">แผนการเรียนที่รับสมัคร <span className="text-destructive">*</span></label>
             <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 rounded-md border border-input bg-background p-4">
              {plansDB.length === 0 ? (
                ...
                plansDB.map((plan) => (
                  <label key={plan.id} className="flex items-center gap-2 text-sm cursor-pointer hover:bg-muted p-1 rounded transition-colors text-foreground">
                    <input
                      type="checkbox"
            ...
            {criteriaForm.study_plans.length === 0 && (
               <p className="mt-1 text-xs text-destructive">กรุณาเลือกแผนการเรียนอย่างน้อย 1 แผน</p>
            )}
           </div>

          <div className="border-t border-border pt-4">
             <label className="mb-2 block text-sm font-medium text-foreground">สถานะการศึกษา <span className="text-destructive">*</span></label>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 rounded-md border border-input bg-background p-4">
               {[
                ...
                <label key={option.id} className="flex items-center gap-2 text-sm cursor-pointer hover:bg-muted p-1 rounded transition-colors text-foreground">
                   <input
                    type="checkbox"
            ...
            {!(criteriaForm.edu_status_req || []).some(req => ["studying", "graduated"].includes(req)) && (
               <p className="mt-1 text-xs text-destructive">กรุณาเลือกสถานะอย่างน้อย 1 รายการ</p>
            )}
          </div>

          <div className="pt-2">
            <label className="mb-2 block text-sm font-medium text-foreground">วุฒิการศึกษาที่รองรับ <span className="text-destructive">*</span></label>
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 rounded-md border border-input bg-background p-4">
              {[
             ...
             {!(criteriaForm.edu_status_req || []).some(req => ["high-school", "vocational", "high-vocational"].includes(req)) && (
              <p className="mt-1 text-xs text-destructive">กรุณาเลือกวุฒิการศึกษาอย่างน้อย 1 รายการ</p>
             )}
          </div>

          <div className="border-t border-border pt-4">
             <div className="mb-2 flex items-center justify-between">
              <label className="block text-sm font-medium text-foreground">เกณฑ์คะแนนที่ใช้พิจารณา (TCAS Subjects)</label>
            ...
            <div className="space-y-2">
               {criteriaForm.subjects.length === 0 ? (
                 <div className="text-sm text-muted-foreground p-4 text-center border border-dashed border-border rounded-md bg-muted/50">
                   ไม่มีการระบุคะแนนสอบ (พิจารณาจากแฟ้มสะสมผลงาน / GPAX เท่านั้น)
                 </div>
               ) : (
                criteriaForm.subjects.map((sub, index) => (
                   <div key={index} className="flex flex-col sm:flex-row gap-2 items-start sm:items-center bg-muted/50 p-2 rounded-md border border-border">
                    <div className="flex-1 w-full">
                      <select required value={sub.subject_id} onChange={(e) => updateSubjectRow(index, "subject_id", e.target.value)} className="h-9 w-full rounded-md border border-input bg-background text-foreground px-3 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary">
                         <option value="" disabled>เลือกวิชา...</option>
                        {subjectsDB.map(s => <option key={s.id} value={s.id}>{s.subject_name}</option>)}
                      </select>
                    </div>
                     <div className="w-full sm:w-32">
                      <input type="number" step="0.01" min="0" max="100" placeholder="ขั้นต่ำ (ถ้ามี)" value={sub.min_score} onChange={(e) => updateSubjectRow(index, "min_score", e.target.value)} className="h-9 w-full rounded-md border border-input bg-background text-foreground px-3 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary" />
                     </div>
                    <div className="w-full sm:w-32 flex items-center gap-2">
                       <input type="number" step="0.01" min="0" max="100" placeholder="ค่าน้ำหนัก %" value={sub.weight} onChange={(e) => updateSubjectRow(index, "weight", e.target.value)} className="h-9 w-full rounded-md border border-input bg-background text-foreground px-3 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary" />
                      <button type="button" onClick={() => removeSubjectRow(index)} className="p-1.5 text-destructive hover:bg-destructive/10 rounded-md transition-colors">
          ...
          <div className="border-t border-border pt-4">
             <label className="mb-1 block text-sm font-medium text-foreground">โครงการรับเข้าศึกษา</label>
            <select required value={criteriaForm.project_id} onChange={e => setCriteriaForm({...criteriaForm, project_id: e.target.value})} className="h-10 w-full rounded-md border border-input bg-background text-foreground px-3 outline-none focus:border-primary focus:ring-1 focus:ring-primary">
           ...
           <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
               <label className="mb-1 block text-sm font-medium text-foreground">คณะ</label>
              <select required value={criteriaForm.faculty_id} onChange={e => setCriteriaForm({...criteriaForm, faculty_id: e.target.value, dept_id: "", program_id: ""})} className="h-10 w-full rounded-md border border-input bg-background text-foreground px-3 outline-none focus:border-primary focus:ring-1 focus:ring-primary">
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-foreground">สาขาวิชา</label>
               <select required disabled={!criteriaForm.dept_id} value={criteriaForm.program_id} onChange={e => setCriteriaForm({...criteriaForm, program_id: e.target.value})} className="h-10 w-full rounded-md border border-input bg-background text-foreground px-3 disabled:opacity-50 outline-none focus:border-primary focus:ring-1 focus:ring-primary">
              ...
              <div key={crit.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between border border-border rounded-lg p-4 bg-background">
                 ...
                 <div className="flex gap-2 w-full sm:w-auto">
                  <button 
                    ...
                    className="flex-1 sm:flex-none flex justify-center items-center gap-1 rounded-md bg-amber-500/10 text-amber-500 px-3 py-1.5 text-sm font-medium hover:bg-amber-500/20 transition-colors"
                  >
                     <Pencil className="h-4 w-4" /> แก้ไข
                  </button>
                   <button 
                    onClick={() => handleDelete(crit.id)} 
                     className="flex-1 sm:flex-none flex justify-center items-center gap-1 rounded-md bg-destructive/10 text-destructive px-3 py-1.5 text-sm font-medium hover:bg-destructive/20 transition-colors"
                  >
